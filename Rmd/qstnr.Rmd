    ...
    dplyr::group_modify(\(d, k) {
      ...
      d_new <-
        ...
        purrr::map(\(id) {
          ...
          if (id == "values") {
            return(d |> dplyr::mutate(value_scale,
                                      values,
                                      value_sets = list(rep("values", length(d$values[[1L]]$int))),
                                      .keep = "none"))
          }
          # handle external value sets
          val_set <- val_set(id = id,
                             survey_config = survey_config)
          gen_qstnr_row(val_set)
        }) |>
        purrr::reduce(\(d1, d2) {
          
          d1$values %<>% purrr::list_merge(!!!list(c(set_id = d1$value_sets)))
          d2$values %<>% purrr::list_merge(!!!list(c(set_id = rep(d2$id, length(d2$values[[1L]]$int)))))
